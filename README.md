...
The project root contains config files ~ git, prettier, eslint. Some build / integration files, eg . docker compose, dev containers etc & last of all the src/ of my java + Angular monolith `/src/*`.

############
############ 
LOOK INTO YEOMAN CONFIG FILES
##############################
##############################
...
To utilise these tools it is nescessary to have [Keycloak](https://keycloak.org) up and running.
The security settings in `src/main/resources/config/application.yml` are configured for this use case.
...
```
A portion of Keycloaks configuration is done during build time, and the rest is completed post initialisation [list] See [link here](https://www.keycloak.org/server/all-config) for all build + config options.
Before switching to production [READ HERE](https://www.keycloak.org/server/configuration).
###############################
# Alternatives to Keycloack exist, Okta is also a very popular and secure way to exhange credentials via 0AUTH2
### Okta utlise a cli and can be found here :) [Okta CLI](https://cli.okta.com/) . Once installed run
...
# Redis as a cache, you will have to launch a cache server.
...
To build the final jar and optimize the SysAdminApp application for production, run:
